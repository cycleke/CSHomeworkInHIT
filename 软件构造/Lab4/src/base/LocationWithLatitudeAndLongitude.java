...
/**
 * This class {@link LocationWithLatitudeAndLongitude} is an implementation of
 * {@link Location} with latitude and longitude.
 *
 * @author cycleke
...
public class LocationWithLatitudeAndLongitude extends CommonLocation {
  ...
  // Rep invariant:
  // name is not null, and -90 <= latitude <= 90, -180 <= longitude <= 180, or
  // they are both NaN
  // Safety from rep exposure:
  // The field are all private and final. There are only getters, no setters.

  /**
   * Constructor of {@link LocationWithLatitudeAndLongitude}, if the latitude and
   * longitude are unknown.
   *
   ...
   * @param name      the name of the location, not null
   * @param shareable whether the location is shareable
   * @param latitude  the latitude of the location, -90 <= latitude <= 90
   * @param longitude the longitude of the location, -180 <= longitude <= 180
   */
  ...
  /**
   * Obtains an instance of {@link LocationWithLatitudeAndLongitude}, if the
  ...
  public String toString() {
    if (Double.isNaN(latitude)) {
      ...
      if (latitude == 0) {
        ...
        northOrSouth = this.latitude > 0 ? "°N" : "°S";
      }
      if (longitude == 0) {
        eastOrWest = "°";
      } else {
      ...
      return getName() + String.format("(%f%s, %f%s)", latitude, northOrSouth, longitude, eastOrWest);
    }
  }
}
