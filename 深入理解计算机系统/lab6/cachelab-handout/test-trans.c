...
void eval_perf(unsigned int s, unsigned int E, unsigned int b) {
  ...
  for (i = 0; i < func_counter; i++) {
    ...
    printf("\nFunction %d (%d total)\nStep 1: Validating and generating memory "
           "traces\n",
           i, func_counter);
    /* Use valgrind to generate the trace */

    sprintf(cmd,
            "valgrind --tool=lackey --trace-mem=yes --log-fd=1 -v ./tracegen "
            "-M %d -N %d -F %d  > trace.tmp",
            M, N, i);
    flag = WEXITSTATUS(system(cmd));
    if (0 != flag) {
      printf(
          "Validation error at function %d! Run ./tracegen -M %d -N %d -F %d "
    ...
    if (results.funcid == i) { results.correct = 1; }
    ...
    while (fgets(buf, 1000, full_trace_fp) != NULL) {
      ...
      if (buf[0] == ' ' && buf[2] == ' ' &&
        ...
        if (flag && addr < 0xffffffff) { fputs(buf, part_trace_fp); }
    ...
    char cmd[255];
    sprintf(cmd, "./csim-ref -s %u -E %u -b %u -t trace.f%d > /dev/null", s, E,
            b, i);
    system(cmd);
    ...
    printf("func %u (%s): hits:%u, misses:%u, evictions:%u\n", i,
           func_list[i].description, hits, misses, evictions);

    /* If it is transpose_submit(), record number of misses */
    if (results.funcid == i) { results.misses = misses; }
  }
}
