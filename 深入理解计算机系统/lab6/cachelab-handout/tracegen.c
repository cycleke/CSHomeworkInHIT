...
int validate(int fn, int M, int N, int A[N][M], int B[M][N]) {
  ...
  for (int i = 0; i < M; i++) {
    for (int j = 0; j < N; j++) {
      if (B[i][j] != C[i][j]) {
        printf("Validation failed on function %d! Expected %d but got %d at "
               "B[%d][%d]\n",
               fn, C[i][j], B[i][j], i, j);
        return 0;
      }
...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt(argc, argv, "M:N:F:")) != -1) {
    ...
    case 'F': selectedFunc = atoi(optarg); break;
    case '?':
    default: printf("./tracegen failed to parse its options.\n"); exit(1);
    }
  }
  ...
  assert(marker_fp);
  fprintf(marker_fp, "%llx %llx", (unsigned long long int)&MARKER_START,
  ...
  if (-1 == selectedFunc) {
    ...
    for (i = 0; i < func_counter; i++) {
      ...
      MARKER_END = 34;
      if (!validate(i, M, N, A, B)) return i + 1;
    }
  } else {
    MARKER_START = 33;
    (*func_list[selectedFunc].func_ptr)(M, N, A, B);
    MARKER_END = 34;
    if (!validate(selectedFunc, M, N, A, B)) return selectedFunc + 1;
  }
  return 0;
}
