...
void eval(char *cmdline) {
  ...
  if (!builtin_cmd(argv)) {
    ...
    if (sigaddset(&mask, SIGINT)) unix_error("sigaddset error");
    if (sigaddset(&mask, SIGTSTP)) unix_error("sigaddset error");
 ...
 */
int builtin_cmd(char **argv) {
  if (!strcmp(argv[0], "quit")) { /* quit指令 exit(0); */
    exit(0);
  } else if (!strcmp(argv[0], "&")) { /* 单独的& */
    return 1;
  } else if (!strcmp(argv[0], "jobs")) { /* jobs指令 */
    sigset_t mask, old_mask;
    if (sigprocmask(SIG_BLOCK, &mask, &old_mask) < 0) /* 阻塞信号 */
      unix_error("sigprocmask error");
    listjobs(jobs);
    sigprocmask(SIG_SETMASK, &old_mask, NULL); /* 恢复信号 */
    return 1;
  } else if (!strcmp(argv[0], "fg") || !strcmp(argv[0], "bg")) { /* fg/bg指令 */
    do_bgfg(argv);
    return 1;
  }
 ...
 */
void waitfg(pid_t pid) {
  while (pid == fgpid(jobs)) { sleep(0); /* busy loop */ }
  return;
}
...
void sigchld_handler(int sig) {
  ...
  if (sigfillset(&mask) < 0) unix_error("sigfilldset error");

  while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
    jobp = getjobpid(jobs, pid); /* 获取子进程对应的job_t */
    if (sigprocmask(SIG_BLOCK, &mask, &old_mask) <
        0) /* 阻塞信号，便于删除进程 */
      unix_error("sigprocmask error");

    if (WIFSTOPPED(status)) { /* 子进程已停止 */
      jobp->state = ST;
...
void sigint_handler(int sig) {
  ...
  if (sigfillset(&mask) < 0) unix_error("sigfillset error");
  if (sigprocmask(SIG_BLOCK, &mask, &old_mask) < 0) /* 阻塞信号 */
    unix_error("sigprocmask error");

  pid = fgpid(jobs); /* 获取前台进程 */

  sigprocmask(SIG_SETMASK, &old_mask, NULL); /* 恢复信号 */

  if (pid && kill(-pid, SIGINT)) /* 停止进程 */
