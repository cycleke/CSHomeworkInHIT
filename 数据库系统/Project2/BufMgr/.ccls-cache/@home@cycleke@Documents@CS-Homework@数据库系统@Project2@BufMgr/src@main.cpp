...
int main()
{
  std::cout << "Start BufMgr Program" << std::endl;
...
void test7() {
  ...
  try {
    bufMgr->flushFile(file1ptr);
    PRINT_ERROR("ERROR :: Pages pinned for file being flushed. Exception "
                "should have been thrown before execution reaches this point.");
  } catch (PagePinnedException &e) {}

  for (i = 0; i < num; i++) { bufMgr->unPinPage(file1ptr, pid[i], true); }
...
void test8() {
  ...
  sprintf((char *)tmpbuf, "test.8 Page %d %7.1f", i, (float)i);
